...
public class TicketView implements ActionListener{
	...
	private String[] column = {"Movie Name", "Ticket Number", "Show Time","Quantity", "Price"};
	...
	public TicketView(String user)
	{
        ...
        String file = "files/TicketView.txt";
        ...
        try {
            ...
            for (int i = 0; i < totalLines; i++)
			{
				...
				{   
                    ...
                    rows[1] = Files.readAllLines(Paths.get(file)).get((i + 1)).substring("TicketNumber:".length());
                    rows[2] = Files.readAllLines(Paths.get(file)).get((i + 3)).substring("ShowTime:".length());
					rows[3] = Files.readAllLines(Paths.get(file)).get((i + 4)).substring("Quantity:".length());
					rows[4] = Files.readAllLines(Paths.get(file)).get((i + 5)).substring("TicketPrice:".length())+"Tk";
                    model.addRow(rows);
